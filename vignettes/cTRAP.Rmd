...
perturbations using the available methods (Spearman's correlation, Pearson's 
correlation and Gene Set Enrichment Analysis (GSEA)). For GSEA, the top and 
bottom *n* genes from the user's differential expression profile are used as
gene sets (by default, *n* = 150 genes).
...
# Compare against CMap using Spearman's and Pearson's correlation and gene set
# enrichment analysis (GSEA) with the top and bottom 150 genes
compareCompounds <- compareAgainstCMap(diffExprStat, cmapPerturbationsCompounds)
```
...
compareCompounds[order(spearman_rank, na.last=NA)]
compareCompounds[order(pearson_rank, na.last=NA)]
compareCompounds[order(gsea_rank, na.last=NA)]

plot(compareCompounds, "spearman")
plot(compareCompounds, "pearson")
plot(compareCompounds, "gsea")
```
...
> To calculate the GSEA score, GSEA is run for top and bottom ranked genes from 
the user's differential expression profile. The GSEA score is the mean between 
ES~top~ and ES~bottom~ (however, if ES~top~ and ES~bottom~ have the same sign, 
the GSEA score is 0).

> If a perturbation has a similar differential expression profile to our data 
(higher GSEA score), we expect to see the top (bottom) ranked genes in the
perturbation enriched in the top (bottom) *n* differentially expressed genes
from our data.
...
# correlation)
print(compareCompounds[order(spearman_rank, na.last=NA)], 1)
```
