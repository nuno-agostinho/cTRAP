...
cell line and load associated gene information and differential gene expression
data. Data is loaded based on the given filename (if no file with the given 
filename is available, data will be first downloaded).
...
for more details). As this file's size is big (around 20GB), a prompt will ask
you to confirm whether to start downloading the file.
>
> Alternatively, you can download the file from the following link (do not
forget to give the correct filename):
ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE92nnn/GSE92742/suppl/GSE92742_Broad_LINCS_Level5_COMPZ.MODZ_n473647x12328.gctx.gz
...
If the interest is in small molecules, the differential gene expression z-scores
from CMap can be downloaded for each small molecule perturbation:
...
```{r}
assoc     <- loadExpressionDrugSensitivityAssociation("CTRP 2.1")
