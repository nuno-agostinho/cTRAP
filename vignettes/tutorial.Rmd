...
To load the `l1000` package into your R environment type:
...
query_results <- queryEncode(
    df=encode_df, organism="Homo sapiens", lab="ENCODE Processing Pipeline",
    biosample_name=cellLine, target=gene, 
    assay=c("shRNA RNA-seq", "siRNA RNA-seq"), fixed=TRUE)
query_results <- query_results[
    query_results$output_type == "gene quantifications", ]
...
# 6th, 1st and 1st most positively associated perturbation based on Spearman
...
plot(compareKnockdown$gsea, EIF4G1knockdown, topGenes=TRUE)
plot(compareKnockdown$gsea, EIF4G1knockdown, topGenes=FALSE)
...
plot(compareKnockdown$gsea,
     tail(compareKnockdown$gsea_ordered, 1)$genes, topGenes=TRUE)
plot(compareKnockdown$gsea, tail(compareKnockdown$gsea_ordered, 1)$genes,
     topGenes=FALSE)
```
