...
cmapMetadataCompound <- filterCMapMetadata(
    cmapMetadata, cellLine="HepG2", perturbationType="Compound")

cmapPerturbationsCompound <- loadCMapPerturbations(
    metadata=cmapMetadataCompound, zscores="cmapZscores.gctx",
...
data("cmapPerturbationsCompound")
```
...
# Compare against CMap using Spearman's and Pearson's correlation and gene set
# enrichment analysis (GSEA) with the top and bottom 150 genes
compareCompound <- compareAgainstCMap(diffExprStat, cmapPerturbationsCompound)
```
...
head(compareKD[order(spearman_coef_rank)], n=10)
head(compareKD[order(pearson_coef_rank)])
head(compareKD[order(WTCS_rank)])
...
compareCompound[order(spearman_coef_rank)]
compareCompound[order(pearson_coef_rank)]
compareCompound[order(WTCS_rank)]

plot(compareCompound, "spearman")
plot(compareCompound, "pearson")
plot(compareCompound, "gsea")
```
...
print(compareCompound[order(spearman_coef_rank)], 1)
```
...
plot(cmapPerturbationsKD, 
     tail(compareKD[order(spearman_coef_rank)], 1), diffExprStat, "spearman")
plot(cmapPerturbationsKD, 
     tail(compareKD[order(pearson_coef_rank)], 1), diffExprStat, "pearson")
plot(cmapPerturbationsKD, 
     tail(compareKD[order(WTCS_rank)], 1), diffExprStat, "gsea")
```
...
plot(cmapPerturbationsCompound, 
     head(compareCompound[order(spearman_coef_rank)], 1), diffExprStat,
     "spearman")
plot(cmapPerturbationsCompound,
     head(compareCompound[order(pearson_coef_rank)], 1), diffExprStat, 
     "pearson")
plot(cmapPerturbationsCompound, 
     head(compareCompound[order(WTCS_rank)], 1), diffExprStat, "gsea")

# Plot relationship with most negatively associated perturbation
plot(cmapPerturbationsCompound,
     tail(compareCompound[order(spearman_coef_rank)], 1), diffExprStat, 
     "spearman")
plot(cmapPerturbationsCompound, 
     tail(compareCompound[order(pearson_coef_rank)], 1), diffExprStat,
     "pearson")
plot(cmapPerturbationsCompound, 
     tail(compareCompound[order(WTCS_rank)], 1), diffExprStat, "gsea")
```
