...
zscoresKD <- loadCMapData("cmapZscores.gctx", "zscores", cmapMetadataKD$sig_id)

cmapPerturbations_HepG2_KD <- loadCMapPerturbations(
  metadata=cmapMetadataKD, zscoresKD, geneInfo="cmapGeneInfo.txt")
```
...
zscoresSmallMolecules <- loadCMapData("cmapZscores.gctx", "zscores",
                                      cmapMetadataSmallMolecules$sig_id)

cmapPerturbations_HepG2_SmallMolecules <- loadCMapPerturbations(
    metadata=cmapMetadataSmallMolecules, zscoresSmallMolecules,
...
The `compareAgainstCMap` function compares the statistic for differential
...
# enrichment analysis (GSEA) with the top and bottom 150 genes
compareKD <- compareAgainstCMap(diffExprStat, cmapPerturbations_HepG2_KD)
```
...
# Compare against CMap using Spearman's and Pearson's correlation and gene set
# enrichment analysis (GSEA) with the top and bottom 150 genes
compareSmallMolecule <- compareAgainstCMap(
  diffExprStat, cmapPerturbations_HepG2_SmallMolecules)
```
...
Pearson's correlation coefficient and/or GSEA Enrichment Score), the respective
p-value and the Benjamini-Hochberg-adjusted p-value.

```{r order knockdown perturbation comparison}
# 10 most positively associated perturbations (note that EIF4G1 knockdown is the
...
head(compareKD[order(spearman_coef_rank)], 10)
head(compareKD[order(pearson_coef_rank)], 10)
head(compareKD[order(WTCS_rank)], 10)

# Most negatively associated perturbations
tail(compareKD[order(spearman_coef_rank)])
tail(compareKD[order(pearson_coef_rank)])
tail(compareKD[order(WTCS_rank)])

# Plot list of pertubations
plot(compareKD, "spearman")
```
...
compareSmallMolecule[order(spearman_coef_rank)]
compareSmallMolecule[order(pearson_coef_rank)]
compareSmallMolecule[order(WTCS_rank)]
...
EIF4G1knockdown <- grep("EIF4G1", compareKD$gene_perturbation, value=TRUE)
print(compareKD, EIF4G1knockdown)

# Information on the most similar compound perturbation as ranked by Spearman's
print(compareSmallMolecule[order(spearman_coef_rank)], 1)
```
...
plot(zscoresKD, EIF4G1knockdown, "spearman")
plot(zscoresKD, EIF4G1knockdown, "pearson")
plot(zscoresKD, EIF4G1knockdown, "gsea")

# Plot relationship with most negatively associated perturbation
plot(zscoresKD, tail(compareKD[order(spearman_coef_rank)], 1))
plot(zscoresKD, tail(compareKD[order(pearson_coef_rank)], 1))
plot(zscoresKD, tail(compareKD[order(WTCS_rank)], 1))
```

> By default, `plot()` plots both the top and bottom genes based on the
`compareAgainstCMap()` arguments.
...
plot(zscoresSmallMolecules, 
     head(compareSmallMolecule[order(spearman_coef_rank)], 1))
plot(zscoresSmallMolecules,
     head(compareSmallMolecule[order(pearson_coef_rank)], 1))
plot(zscoresSmallMolecules, 
     head(compareSmallMolecule[order(WTCS_rank)], 1))

# Plot relationship with most negatively associated perturbation
plot(zscoresSmallMolecules,
     tail(compareSmallMolecule[order(spearman_coef_rank)], 1))
plot(zscoresSmallMolecules, 
     tail(compareSmallMolecule[order(pearson_coef_rank)], 1))
plot(zscoresSmallMolecules, 
     tail(compareSmallMolecule[order(WTCS_rank)], 1))
```
